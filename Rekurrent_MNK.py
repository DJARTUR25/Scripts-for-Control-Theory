...
def R_LQM():     
    ...
    H0 = RM[:2].transpose() @ RM[:2]                 # промежуточная матрица; начальная; считаем как C0^T * C0
    ...
    for k in range(2, n):
        ...
        x0 = x0 + np.linalg.inv(H0) @ RM[k, :].transpose() * (ylst[k] - RM[k, :].transpose() @ x0)  # вычисление нового x_k
